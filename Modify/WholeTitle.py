...
def procedureTitle(str:str):
    ...
    SearchTitleWithWords(Set,CopyTitle,Dictionary,DictionaryOrigin)
    Display(Dictionary)
...
#start searching
def SearchTitleWithWords(Set,Dictionary,Origin,ORigin):
    ...
    MatchWithOriginDic(Origin,ORigin)

def MatchWithOriginDic(Dic,DicOri):
    KeysD = list(Dic.keys())
    KeysDO = list(DicOri.keys())
    for i in range(0,len(KeysD)):
        if Correspond[0] == KeysD[i]:
            Correspond[0] = KeysDO[i]
        if Correspond[1] == KeysD[i]:
            Correspond[1] = KeysDO[i]
        if Correspond[2] == KeysD[i]:
            Correspond[2] = KeysDO[i]
