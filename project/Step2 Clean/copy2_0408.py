...
def Clean(BigDic):
    ...
    formatNum = re.compile(r"[1-9]\.[1-9]?")
    formatSign = re.compile(r"；|。|：")
    formatSpace = re.compile(r"\s")
    for i in range(0,len(KeyList)):
        OrderList = list(BigDic[KeyList[i]].keys())
        for j in range(0,len(OrderList)):
            for text in BigDic[KeyList[i]][OrderList[j]]:
                print(re.match(formatSign,text))
                print(re.match(formatNum,text))
                print(re.match(formatChap,text))
                print(re.match(formatSpace,text))
