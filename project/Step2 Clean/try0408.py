...
'''for i in range(0,len(DicKeys)):
        for j in range(0,len(list(Dic[DicKeys[i]]))):
            for k in range(0,len(Dic[DicKeys[i]][j].values())):
                if(re.match(formatNum,Dic[DicKeys[i]][j][k])):
                    Dic[DicKeys[i]][j][k].rspilt('0123456789')
                if(re.match(formatSign,Dic[DicKeys[i]][j][k])):
                    Dic[DicKeys[i]][j][k].spilt('。；：')'''
                    
                    
'''DicKeys = list(Dic.keys())
    formatNum = re.compile(r"[0-9]\.[0-9]?.+")          #首
    formatSign = re.compile(r".+'：'|'；'|'。'")        #尾
    formatSpace = re.compile(r"\s")                     #中
    return Dic '''